...
```{r full-map, fig.width=10, fig.height=7}
#| lightbox: true
#| fig-cap: "계면 임피던스 매칭의 완전한 지도: 디스플레이 → AI 소프트웨어 → AI 반도체"

df_fullmap <- data.frame(
  x = c(1.5, 4, 6.5,    1, 4, 7,    2.5, 5.5),
  y = c(5, 5, 5,         3, 3, 3,    1, 1),
  label = c(
    "OLED TFT\nSiOx/poly-Si\nε·ρ 매칭",
    "딥 뉴럴넷\n가중치 초기화\nσ²w 매칭",
  ...
  domain = c("디스플레이", "AI 소프트웨어", "AI 패키징",
             "디스플레이", "AI 하드웨어", "AI 패키징",
             "뉴로모픽", "AI 하드웨어"),
  era = c("2018", "2017", "2024", "1990s", "2025", "2025", "2021", "2024")
)

ggplot(df_fullmap, aes(x = x, y = y)) +
  # Connection arrows - top row
  geom_segment(aes(x = 1.5, xend = 4, y = 5, yend = 5),
               arrow = arrow(length = unit(0.15, "cm"), ends = "both"),
               color = "#6366f1", linewidth = 0.6) +
  geom_segment(aes(x = 4, xend = 6.5, y = 5, yend = 5),
               arrow = arrow(length = unit(0.15, "cm"), ends = "both"),
               color = "#6366f1", linewidth = 0.6) +
  # Vertical connections
  geom_segment(aes(x = 1.5, xend = 1, y = 5, yend = 3),
               color = "#888888", linewidth = 0.3, linetype = "dotted") +
  geom_segment(aes(x = 4, xend = 4, y = 5, yend = 3),
               color = "#888888", linewidth = 0.3, linetype = "dotted") +
  geom_segment(aes(x = 6.5, xend = 7, y = 5, yend = 3),
               color = "#888888", linewidth = 0.3, linetype = "dotted") +
  # Bottom connections
  geom_segment(aes(x = 4, xend = 2.5, y = 3, yend = 1),
               color = "#888888", linewidth = 0.3, linetype = "dotted") +
  geom_segment(aes(x = 4, xend = 5.5, y = 3, yend = 1),
               color = "#888888", linewidth = 0.3, linetype = "dotted") +
  # Points
  geom_point(aes(fill = domain), shape = 21, size = 20, alpha = 0.25, color = NA) +
  geom_text(aes(label = label), color = "#e4e4ec", size = 2.6,
            family = kfont, lineheight = 1.15) +
  # Annotations
  annotate("text", x = 2.75, y = 5.35, label = "수학적 동형",
           color = "#6366f1", size = 2.8, family = kfont, fontface = "bold") +
  annotate("text", x = 5.25, y = 5.35, label = "물리적 동일",
           color = "#6366f1", size = 2.8, family = kfont, fontface = "bold") +
  # Row labels
  annotate("text", x = 0.2, y = 5, label = "핵심\n적층 구조",
           color = "#888888", size = 2.5, family = kfont, hjust = 0) +
  annotate("text", x = 0.2, y = 3, label = "선행\n기술",
           color = "#888888", size = 2.5, family = kfont, hjust = 0) +
  annotate("text", x = 0.2, y = 1, label = "차세대\n응용",
           color = "#888888", size = 2.5, family = kfont, hjust = 0) +
  scale_fill_manual(values = c("#ef4444", "#6366f1", "#22c55e",
                               "#f59e0b", "#8b5cf6")) +
  xlim(-0.3, 8) + ylim(0, 6) +
  labs(
    title = "계면 임피던스 매칭의 완전한 지도",
    subtitle = "디스플레이 → AI 소프트웨어 → AI 반도체: 같은 물리, 같은 방정식, 같은 팹"
  ) +
  theme_insight() +
  theme(axis.text = element_blank(), axis.title = element_blank(),
        panel.grid = element_blank(),
...
같은 삼성 용인 캠퍼스에서 OLED 패널의 게이트 절연막 레시피를 최적화하는 엔지니어와, HBM의 TSV 임피던스를 튜닝하는 엔지니어는 — 서로 모르지만 — **같은 방정식의 같은 해**를 찾고 있다.
...
이 조건은 OLED 게이트 스택에서도, 10,000층 CNN에서도, HBM 16-다이 적층에서도, 멤리스터 크로스바 어레이에서도 동일하게 작동한다.

차이는 목적뿐이다:

- **디스플레이 엔지니어**는 계면 전하를 **없앤다** (매칭 → 잔상 제거)
- **AI 연구자**는 계면 신호를 **안정화한다** (매칭 → 학습 안정)
- **AI 패키징 엔지니어**는 계면 임피던스를 **정밀 제어한다** (매칭 → signal integrity)
- **뉴로모픽 엔지니어**는 계면 전하를 **활용한다** (의도적 비매칭 → 메모리)

그리고 이 네 분야의 엔지니어들은 같은 팹에서, 같은 장비로, 같은 SiOx를 깐다.

Maxwell-Wagner가 1892년에 쓴 방정식이, 130년 뒤 AI 반도체 시대에도 유효하다. 좋은 물리학은 시대를 고르지 않는다.
...
12. Choi, N., "Impedance matching at poly-Si/SiOx interface in OLED TFTs," Working paper, 2026
13. Choi, N., Ph.D. Thesis, Korea University, 2018
:::
