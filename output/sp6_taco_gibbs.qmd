...
subtitle: "깁스 자유에너지로 보는 트럼프 정치경제학"
...
$$\Delta G_{시장} = \Delta H_{정책} - T_{감정} \cdot \Delta S_{무질서}$$
...
for sx, sl in [(2,'2024 유세장 피습'), (5.5,'2025 탄핵 시도'), (9,'2026 만찬장 총격')]:
    ...
    fig1.add_annotation(x=sx, y=2.7, text=f"<b>{sl}</b>", showarrow=False,
                        ...
                        bgcolor="rgba(40,20,20,0.85)", bordercolor=RED, borderwidth=1)

fig1.add_annotation(x=11, y=-2.0, text="<b>감쇠 없음!<br>진폭 계속 증가</b>",
                    showarrow=False, font=dict(size=12, color=RED, family=FONT))

fig1.update_layout(
    height=400, paper_bgcolor=BG, plot_bgcolor=CARD,
    font=dict(family=FONT, color=WHITE),
    xaxis=dict(title="시간 →", gridcolor='#222'),
    yaxis=dict(title="정치적 강도 (진폭)", gridcolor='#222'),
    legend=dict(bgcolor='rgba(20,20,40,0.8)', bordercolor='#333'),
...
fig2.update_layout(
    height=450, paper_bgcolor=BG, plot_bgcolor=CARD,
    font=dict(family=FONT, color=WHITE),
    xaxis=dict(title="T (사회적 온도 / 감정 열기)", gridcolor='#222', range=[0,5.5]),
    yaxis=dict(title="S (시스템 엔트로피 / 무질서도)", gridcolor='#222', range=[0,5.5]),
    legend=dict(bgcolor='rgba(20,20,40,0.8)', bordercolor='#333'),
    margin=dict(t=30, b=50),
)
fig2.show()
...
for lt, ll in [(1,'관세'), (3,'딜'), (5,'이란 전쟁'), (7,'만찬 총격')]:
    fig3.add_vline(x=lt, line=dict(color='#444', width=1, dash='dot'))
...
fig3.add_annotation(x=9, y=-2.5, text="<b>ΔG < 0</b><br>자발적 변화 영역",
    ...
    bgcolor="rgba(20,40,20,0.85)", bordercolor=GREEN, borderwidth=1)

fig3.update_layout(
    height=400, paper_bgcolor=BG, plot_bgcolor=CARD,
    font=dict(family=FONT, color=WHITE),
    xaxis=dict(title="시간 →", gridcolor='#222'),
    yaxis=dict(title="에너지", gridcolor='#222'),
    legend=dict(bgcolor='rgba(20,20,40,0.8)', bordercolor='#333'),
    margin=dict(t=30, b=50),
)
fig3.show()
...
for c in range(4):
    fig4.add_vline(x=c*2.5, line=dict(color='#333', width=1, dash='dot'))
...
fig4.update_layout(
    height=400, paper_bgcolor=BG, plot_bgcolor=CARD,
    font=dict(family=FONT, color=WHITE),
    xaxis=dict(title="시간 (TACO 사이클 반복)", gridcolor='#222'),
    yaxis=dict(title="누적 엔트로피 (S)", gridcolor='#222'),
    legend=dict(bgcolor='rgba(20,20,40,0.8)', bordercolor='#333'),
    margin=dict(t=30, b=50),
)
fig4.show()
...
fig5.add_trace(go.Table(
    ...
    cells=dict(
        ...
        fill_color=[
            ['rgba(239,68,68,0.2)','rgba(245,158,11,0.15)','rgba(34,197,94,0.15)','rgba(99,102,241,0.15)'],
        ] * 6,
        font=dict(size=11, color='#ddd', family=FONT),
...
for i, (val, col, name, rng) in enumerate(gauges):
    fig6.add_trace(go.Indicator(
        ...
        gauge=dict(
            axis=dict(range=rng, tickcolor='#444'),
            bar=dict(color=col),
            bgcolor=CARD,
...
fig6.update_layout(
    height=280, paper_bgcolor=BG, plot_bgcolor=CARD,
    font=dict(family=FONT, color=WHITE, size=12),
    margin=dict(t=50, b=20),
)
fig6.update_annotations(font=dict(size=12, color=WHITE, family=FONT))
fig6.show()
```
...
$$\Delta G_{민주주의} = \Delta H_{시민행동} - T_{사회열기} \cdot \Delta S_{무질서} < 0$$
